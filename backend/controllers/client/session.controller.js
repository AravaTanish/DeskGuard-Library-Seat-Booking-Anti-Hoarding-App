...
export const logoutSession = asyncHandler(async (req, res) => {
  ...
  res.clearCookie("sessionAccessToken", cookieOptions);
  res.clearCookie("sessionRefreshToken", cookieOptions);
...
export const breakSession = asyncHandler(async (req, res) => {
  const sessionId = req.user.id;
  if (!sessionId) {
    throw new AppError("Invalid sessionId", 400);
  }
  ...
  if (session.isOnBreak) {
    throw new AppError("Break already active", 400);
  }

  session.breaksCount = Number(session.breaksCount || 0) + 1;
  session.breakStartTime = new Date();
  session.breakEndTime = new Date(Date.now() + 20 * 60 * 1000);
  session.isOnBreak = true;

  const computer = session.computerId;
  await computer.populate("adminId", "email");
  computer.status = "break";
  await computer.save();
...
export const endBreak = asyncHandler(async (req, res) => {
  ...
  const session = await Session.findById(sessionId).populate("computerId");
  if (!session) {
    throw new AppError("Session not found", 404);
  }
  if (!session.isOnBreak) {
    throw new AppError("Break already inactive", 400);
  }
